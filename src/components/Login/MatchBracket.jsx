...
const MatchBracket = React.memo((props) => { //TO DO: falta hacer el proceso de eliminacion y verificar el mecanismo de Editar
  const navigate = useNavigate();
  const { tournamentName, tournamentId } = useParams();
  const theme = useTheme();

  const {matches, onwerTournament} = props;
  ...
  const handleEdit = (match) => { // Cambio de vista a Edit form
    navigate(`/match/${tournamentName}/${tournamentId}/${match.myIdMatch}/edit?idB=${match.id}`);
  };
  ...
  const MatchTheme = {
    ...
    connectorColor: theme.palette.mode === "light" ? theme.palette.secondary.light : theme.palette.grey[600],
  ...
  const MyMatch = (props) => {
    ...
    return (
      <Box>
        <Container sx={{display:'flex', justifyContent:'space-between', textAlign: 'center', alignItems: 'center'}}>
        ...
        <CardActionArea onClick={() => {
          ...
          setShowModal(true);}}>
            ...
            <Container 
              ...
              {/* <Typography variant="h6" color={isWinner ? 'success.main' : 'error.main'} sx={{ textAlign: 'center' }}>
                {participants[0]?.resultText+' '+participants[0]?.name} vs {bottomParty?.name}
              </Typography> */}
          </CardActionArea>
          {/* <CardActionArea sx={{ display: 'flex', justifyContent: 'space-around'}}>
            <Fab variant="extended" sx={{width:2, height:25}} color="primary" onClick={() => handleEdit(match.id)}>Edit</Fab>
  ...
  return (
    <>
      ...
      <DialogComponent 
        ...
        buttons={onwerTournament && (
          <>
            <Fab variant="extended" size="small" color="primary" onClick={() => handleEdit(selectedMatch)}>Edit</Fab>
            <Fab variant="extended" size="small" color="error" onClick={() => handleOpenDialog(selectedMatch.myIdMatch)}>Delete</Fab>
          </>
          )}/>
      {matches.length > 0 &&
        ...
        svgWrapper={({ children }) => (
          <Box 
            sx={{
              ...
              display: "block", // Necesario para permitir desplazamiento horizontal
            }}
          >{children}</Box>
        )}
      />}
